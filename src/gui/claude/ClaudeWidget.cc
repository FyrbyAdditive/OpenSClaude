...
QString Widget::getSystemPrompt() const
{
  return QString(
    ...
    "### Extrusions\n"
    "- linear_extrude(height, center, twist, slices, scale) { 2D... }\n"
    "- rotate_extrude(angle, convexity) { 2D... }\n\n"

    "### Advanced Operations\n"
    "- hull() { ... } - convex hull of children\n"
    "- minkowski() { ... } - Minkowski sum of children\n"
    "- offset(r) or offset(delta, chamfer) - expand/contract 2D shapes\n"
    "- projection(cut=false) - 3D to 2D projection\n\n"

    "### Mathematical Functions\n"
    "- Trig: sin, cos, tan, asin, acos, atan, atan2(y,x)\n"
    "- Rounding: abs, ceil, floor, round, sign\n"
    "- Power: ln, log, pow(base,exp), sqrt, exp\n"
    "- Vector: norm(v), cross(v1,v2), min, max\n"
    "- Lookup: lookup(val, table), rands(min,max,count)\n\n"

    "### String & List Functions\n"
    "- str(val) - convert to string\n"
    "- concat(list1, list2) - join lists/strings\n"
    "- len(list_or_string) - length\n"
    "- search(match, string_or_list) - find indices\n"
    "- chr(num) / ord(char) - ASCII conversion\n\n"

    "### List Comprehensions\n"
    "- [for (i = [0:10]) i*i] - basic range\n"
    "- [for (i = list) if (i > 0) i] - with filter\n"
    "- [for (i = [0:5]) let (x = i*2) [x, x+1]] - with let\n"
    "- [each sublist] - flatten nested lists\n\n"

    "### Modifiers (Debug Prefixes)\n"
    "- * - disable this subtree\n"
    "- ! - show only this (root)\n"
    "- # - highlight in transparent red\n"
    "- %% - transparent gray background\n\n"

    "### Special Variables\n"
    "- $fn, $fa, $fs - resolution control\n"
    "- $preview - true in F5, false in F6 render\n"
    "- $t - animation parameter (0.0 to 1.0)\n"
    "- $children - count of children in module\n"
    "- $vpr, $vpt, $vpd - viewport rotation/translation/distance\n\n"

    "### Type Testing\n"
    "- is_undef(x), is_bool(x), is_num(x), is_string(x)\n"
    "- is_list(x), is_function(x)\n\n"

    "### Debugging & Control\n"
    "- echo(\"message\", variable) - print to console\n"
    "- assert(condition, \"message\") - fail if false\n"
    "- render() { ... } - force CGAL evaluation\n"
    "- let(a=1, b=2) expression - local bindings\n"
    ...
    "- children(), children(n) - access child geometry in modules\n"
    "- include <file.scad> - include and execute\n"
    "- use <file.scad> - import modules/functions only\n\n"

    "### Import/Export\n"
    "- import(\"file.stl\") - import STL, OFF, AMF, 3MF, SVG, DXF\n"
    "- surface(\"heightmap.png\", center) - heightmap to surface\n\n"
